...
class HandlerConfig {
	...
	InitHandler initHandler(
		...
		.setEndpoint(() -> "http://localhost:" + FrameworkHelper.getPort(applicationContext) + "/init")
	...
	CacheGroupHandler cacheGroupHandler(
		...
		if (Boolean.TRUE.equals(cache.getUseLocalMemory())) {
			...
			.setExpiryCheckInterval(cache.getExpiryCheckInterval())
			.setTaskHandler(taskHandler);
		} else {
	...
	RestHandler restHandler(
		...
		@Qualifier(AppConstant.Bean.Task.REST)
		TaskHandler taskHandler
	) {
